...
if (
    ...
    st.write("You currently have ",remainUSD," USD and ",remainLTC," LTC.")


    buyAmount = st.number_input("Enter amount of LTC that you want to buy:", min_value=0.000, step=0.001)
    ...
    if st.button("Buy", key='buy'):
        ...
        if (remainUSD >= txnUSD):
            try:
                ...
                queryStmt += "'LTC',"+str(txnLTC)+",'USD',"+str(txnUSD)+","+str(round(buyAmount * buyPrice, 4))+",'"+st.session_state["username"]+"');"
                ...
                try:
                    ...
                    newLTC = remainLTC + txnLTC
                    fee = round(buyAmount * txnPrice, 4)
                    queryStmt = f"UPDATE user_bal SET usd_bal={newUSD}, ltc_bal={newLTC} WHERE username='"+st.session_state["username"]+"';"
    ...
    sellAmount = st.number_input("Enter amount of LTC that you want to sell:", min_value=0.000, step=0.001)
    ...
    if st.button("Sell", key='sell'):
        ...
        if (remainLTC >= txnLTC):
            try:
                queryStmt = "INSERT INTO txn_history (buy_currency, buy_amount, sell_currency, sell_amount, usd_price, username) VALUES ("
                queryStmt += "'USD',"+str(txnUSD)+",'LTC',"+str(txnLTC)+","+str(round(sellAmount * sellPrice, 4))+",'"+st.session_state["username"]+"');"
                run_query(queryStmt,0)
            except:
                st.warning("Error in writing to database for transaction history.")
            else:
            ...
            st.warning("Not enough LTC in your account.")